...
const decisionMap = [
...

	{
		numberMatchedMember: true,
		response: ({ member, message }) => ({
			body: `{${randText('error')}} ${member.firstName}, I don't understand that last message, here are some things you can text me: \n\n` +
				commandsResponse
		})
	},

	{
		...
		response: ({ message }) => ({
			body: `Sorry, we can\'t find your number (${message.fromPhone}) in the ward directory. You should talk to Hamish about this.`,
