...
class Member extends Base {
	getCompanionship() {
		if (!this.attributes.priesthoodOffice ||
			this.attributes.gender == 'FEMALE') {
			return null;
		}
		return Companionship.findByTeacherIndividualId(this.attributes.individualId);
	}
	...
	getAssignedCompanionship() {
		return Companionship.findByAssignmentIndividualId(this.attributes.individualId);
	}

	getHomeTeachers() {
		return co(function *() {
			const assignedCompanionship = yield this.getAssignedCompanionship();
			return assignedCompanionship ? assignedCompanionship.getTeachers() : null;
		}.bind(this));
	}
