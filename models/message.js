...
class Message extends Base {
	...
	getMember() {
		return co(function *() {
			...
			const key = (source === 'inbound') ? 'fromIndividualId' : 'toIndividualId';
			this.cachedRelations.member = yield Member.findOneById(Number(this.attributes[key]));
			return this.cachedRelations.member;
		}.bind(this));
	}
	 ...
	 * @param  {string} ourResponseToMessageSid (optional) The `sid` that this message is a response to
	 *
	 * @return {Message}
	 */
	static createForOutbound(body, type, memberOrPhone, ourResponseToMessageSid) {
		const toIndividualId = (typeof memberOrPhone === "string") ? null : memberOrPhone.id;
		const toPhone = (typeof memberOrPhone === "string") ? memberOrPhone : memberOrPhone.phone;
		const message = new Message({
			...
			status: 'delivered',
			source: 'outbound',
			ourResponseToMessageSid: ourResponseToMessageSid ? ourResponseToMessageSid : null
		});
